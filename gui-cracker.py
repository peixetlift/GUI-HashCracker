...
def decrypt(hashstr):
    ...
    if(algorithm_textbox.get().lower() == 'sha1'):
        result = guidecrypt.guisha1(content, hashstr)
    elif(algorithm_textbox.get().lower() == 'md5'):
        result = guidecrypt.guimd5(content, hashstr)
    elif(algorithm_textbox.get().lower() == 'sha256'):
        result = guidecrypt.guisha256(content, hashstr)
    elif(algorithm_textbox.get().lower() == 'nltm'):
        result = guidecrypt.guinltm(content, hashstr)
...
decrypt_button=tkinter.Button(
    window, image=decrypt_img, padx=5, pady=5, borderwidth=0, command=lambda: decrypt(hash_textbox.get()), bg="#000066", fg="#ffffff")
decrypt_button.grid(row=6, column=3)
encrypt_button=tkinter.Button(
    window, image=encrypt_img, padx=5, pady=5, borderwidth=0, command=lambda: encrypt_word(), bg="#000066", fg="#ffffff")
